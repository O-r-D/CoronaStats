...
class CountryStatsFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val countryStats = arguments?.getParcelable<CountryStats>("tag")
        bindUI(countryStats)

    }

    private fun bindUI(countryStats: CountryStats?) {

        if (countryStats != null) {
            tv_cases_nb.text = String.format("%,d", countryStats.cases)
            if (countryStats.todayCases != 0)
                tv_cases.text = getString(
                    R.string.country_cases,
                    "(${String.format("%,d", countryStats.todayCases)})"
                )

            tv_deaths_nb.text = String.format("%,d", countryStats.deaths)
            if (countryStats.todayDeaths != 0)
                tv_deaths.text = getString(
                    R.string.country_dead,
                    "(${String.format("%,d", countryStats.todayDeaths)})"
                )

            tv_recovered_nb.text = String.format("%,d", countryStats.recovered)

            tv_country_name.text = countryStats.country
        } else {
    ...
    private fun goToCountryListFragment() {
        parentFragmentManager.beginTransaction().apply {
            ...
            TODO("Add to Back Stack for the first fragment only")
        }.commit()
    }
